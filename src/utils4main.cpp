...
void showError(int i, int ac, char **av) {
  (void)ac;
  if (i == 1)
    std::cerr << "\033[31mError: Usage: " << av[0]
              << " <port> <server>\033[0m\n";
  else if (i == 2)
...
int checkport_length(std::string port) {
  if (port.length() > 5) {
    std::cerr
        << "\033[31mError: Port must be equal or less than 5 digits\033[0m\n";
    return 0;
  }
  return 1;
}
...
int check_password(std::string password) {
  ...
  if (password.length() < 8) {
    std::cerr << "\033[31mError: Password must be at least 8 characters "
                 "long\033[0m\n";
    return 0;
  }
  return 1;
